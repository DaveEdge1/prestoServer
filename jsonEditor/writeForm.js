...
function dataTypeHTML (configJSON, key, id1) {
	...
	if (configJSON[key].data_type === 'free-form'){
		...
		+ '<input style:"-moz-appearance: textfield;" class="coord-input" type="number" name="'+id1+'" id="lat_min" value="' + configJSON[key].default[0] + '" min="' + configJSON[key].limits[0] + '" max="' + configJSON[key].limits[1] + '" step="' + configJSON[key].precision + '" onchange="updateRect();">\n'
		+ '<label style="margin-right:8px;">min</label>\n'
		+ '<input class="coord-input" type="number" name="'+id1+'" id="lat_max" value="' + configJSON[key].default[1] + '" min="' + configJSON[key].limits[0] + '" max="' + configJSON[key].limits[1] + '" step="' + configJSON[key].precision + '" onchange="updateRect();">\n'
		+ '<label style="margin-right:30px;">max</label>\n'
		+ '<label style="font-size:16px; margin-right:8px;">Longitude</label>\n'
		+ '<input class="coord-input" type="number" name="'+id1+'" id="lon_min" value="' + configJSON[key].default[2] + '" min="' + configJSON[key].limits[2] + '" max="' + configJSON[key].limits[3] + '" step="' + configJSON[key].precision + '" onchange="updateRect();">\n'
		+ '<label style="margin-right:8px;">min</label>\n'
		+ '<input class="coord-input" type="number" name="'+id1+'" id="lon_max" value="' + configJSON[key].default[3] + '" min="' + configJSON[key].limits[2] + '" max="' + configJSON[key].limits[3] + '" step="' + configJSON[key].precision + '" onchange="updateRect();">\n'
		+ '<label>max</label>\n'
		+ '</div>\n'
		+ '</div>\n'
...
function buildHtml(configs, recon) {
  ...
  return '<!DOCTYPE html>\n'
       ...
       + '<br>\n'
       + '<br>\n'
       + '<p style="color:red; font-size: 14px; text-align:center;">*This form is a demo version. Links in the header as well as the form "submit" button are not functional</p>\n'
       + '<br>\n'
       + '<br>\n'
