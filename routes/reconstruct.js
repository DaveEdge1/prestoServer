...
const reconParams = (recon) => {
  const reconParamsPath = path.join(config.paths.prestoBase, 'presto', 'reconLib.json');
  const reconParamsNow = fs.readFileSync(reconParamsPath, 'utf8');
...
const prestoStartHtml = (uniqueID, user, domain, recon, language) => {
  ...
  return (
    'Starting your custom PReSto ' + params.title + ', ID: ' + uniqueID + '<br /><br />' +
    "WARNING: Using your browser's 'back' button will overwrite your previous submission<br /><br />" +
    '<a href=' + params.github + ' target="_blank">visit the ' + params.title + ' webpage</a><br /><br />' +
