...
python -u /root/presto/viz/1_format_data_daholocene_graphem.py $data_dir 2>&1 | tee -a "$output_dir/viz/1_format_data_full.log"; echo "1_format_data exit code: $? Memory: $(free -h | grep Mem)" >> "$output_dir/viz/1_format_data_full.log" &&
python -u /root/presto/viz/2_make_maps_and_ts.py $data_dir $output_dir 2>&1 | tee -a "$output_dir/viz/2_make_maps_full.log"; echo "2_make_maps exit code: $? Memory: $(free -h | grep Mem)" >> "$output_dir/viz/2_make_maps_full.log" &&
python -u /root/presto/viz/3_make_html_file.py $data_dir $output_dir $web_data_dir 2>&1 | tee -a "$output_dir/viz/3_make_html_full.log"; echo "3_make_html exit code: $? Memory: $(free -h | grep Mem)" >> "$output_dir/viz/3_make_html_full.log" &&
